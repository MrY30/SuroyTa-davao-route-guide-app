...
void main() async {
  ...
  runApp(const ProviderScope(child: SakayTaApp()));
}
...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  maplibreController; // NEW: The native GPU controller
  ...
  String _currentDestinationName =
      ''; // Stores Nominatim name, or empty for manual pins
  ...
  void _clearRoutingData() {
    // 1. Tell Riverpod to wipe the routes
    ref.read(routeStateProvider.notifier).clearRoutingData();

    // 2. THE FIX: Hide the sheet if we are currently on the Locate tab
  ...
  List<Widget> _suggestedRoutesSheet(LatLng? startPin, LatLng? destinationPin, List<RouteResult> suggestedRoutes, List<JeepneyRoute> allRoutes ) {
        ...
        return Card(
          ...
          child: InkWell(
            ...
            onTap: () async {
              ...
              if (result.actualStartWalk == null &&
                  startPin != null &&
                  destinationPin != null) {
                setState(() {
                  result.isFetchingActualRoute = true;
                });

                final routeData = allRoutes.firstWhere(
                  (r) => r.name == result.routeName,
                );
                final boardPoint =
                    routeData.polylineData!.points[result.boardIndex];
                final alightPoint =
                    routeData.polylineData!.points[result.alightIndex];

                // switched to ORS from OSRM
                final startLeg = await _osrmService.fetchWalkingRoute(
                  startPin,
                  boardPoint,
                );
                final endLeg = await _osrmService.fetchWalkingRoute(
                  alightPoint,
                  destinationPin,
                );
                ...
                setState(() {
                  result.isFetchingActualRoute = false;
                });
              }
            },
            child: Container(
              ...
              child: Row(
                children: [
                  isBestRoute
                      ? Icon(Icons.star_outline, size: 35, color: fontColor)
                      : Image.asset(
                          'assets/images/jeep_logo.png',
                          width: 35,
                          height: 35,
                        ),
                  ...
                  Expanded(
                    child: Column(
                      mainAxisAlignment: MainAxisAlignment
                          .center, // Vertically centers the texts inside the fixed 75px height
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Flexible(
                          // Flexible ensures the font shrinks if it hits the edges
                          child: FittedBox(
                            ...
                            child: Text(
                              ...
                              style: TextStyle(
                                ...
                                fontFamily: GoogleFonts.paytoneOne().fontFamily,
                                height:
                                    1.0, // <-- CRITICAL: Removes invisible font padding!
                                shadows: [
                                  Shadow(
  ...
  List<Widget> _buildExploreContent(List<JeepneyRoute> allRoutes) {
    ...
    return [
      Row(
        ...
        children: [
          ...
          GestureDetector(
            onTap: () async {
              // 1. Tell Riverpod to untoggle everything
              await ref.read(routeStateProvider.notifier).toggleAllRoutes(false);
              
              // 2. Update your local UI state
              setState(() {
                _isSelectedRoute = false;
                _isSelectAllChecked = false; // Good practice to clear this too!
              });
              
              // 3. Command MapLibre to erase the lines
              drawMapElements();
            },
            child: AnimatedScale(
      ...
      Card(
        ...
        child: CheckboxListTile(
          ...
          onChanged: (bool? value) async {
            ...
            drawMapElements(); // Redraw the map!
          },
        ),
      ),
      const SizedBox(height: 20),
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            side: BorderSide(
              ...
              width: 2,
            ), // Add a border with the route color
            borderRadius: BorderRadius.circular(12),
          ),
          child: CheckboxListTile(
            title: Text(
      ...
      Container(
        ...
        child: TextField(
          ...
          onSubmitted: (value) async {
            ...
            GeocodeResult? geocodeResult = await _geocodingService
                .searchLocation(value);

            if (geocodeResult != null) {
              ref
                  .read(mapStateProvider.notifier)
                  .setDestinationPin(geocodeResult.coordinates);
              setState(() {
                _clearRoutingData();
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onTap: () {
                  ...
                  ref.read(mapStateProvider.notifier).setDestinationPin(LatLng(fav.latitude, fav.longitude));
                  setState(() {
                      _clearRoutingData();
                      _currentDestinationName = fav.name;
                  });
                  drawMapElements();
                  ...
                  Future.delayed(const Duration(milliseconds: 400), () {
                    maplibreController?.animateCamera(
                      maplibre.CameraUpdate.newLatLngZoom(
                        maplibre.LatLng(fav.latitude, fav.longitude),
                        16.0,
                      ),
                    );
                  });
      ...
      if (historyList.isEmpty)
        ...
        ...historyList.map((historyItem) {
          return ListTile(
            ...
            subtitle: Text(
              '${historyItem.latitude.toStringAsFixed(4)}, ${historyItem.longitude.toStringAsFixed(4)}',
            ...
            onTap: () {
              ...
              ref.read(mapStateProvider.notifier).setDestinationPin(LatLng(historyItem.latitude, historyItem.longitude));
              setState(() {
                  _clearRoutingData();
                  _currentDestinationName = historyItem.name;
              });
  ...
  Future<void> _saveToHistory(String name, LatLng coords) async {
    ...
    final newHistoryItem = FavoriteLocation(
      id: DateTime.now().millisecondsSinceEpoch
          .toString(), // Using timestamp as unique ID
      name: name,
              ...
              return Container(
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (_selectedIndex == 0) ..._buildExploreContent(allRoutes),
                    // if(_selectedIndex == 1) ..._buildLocateContent(),
                    if (_selectedIndex == 1 && suggestedRoutes.isNotEmpty)
                      ..._suggestedRoutesSheet(startPin, destinationPin, suggestedRoutes, allRoutes),
